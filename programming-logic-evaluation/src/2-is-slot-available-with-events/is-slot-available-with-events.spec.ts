...
describe(`02 - ${isSlotAvailableWithEvents.name}`, () => {
  ...
  it('should return true for an available slot without conflicting events', () => {
    ...
    for (const slot of availableSlots) {
      ...
      const expected = true;
      expect(actual).toBe(expected, `Slot with ${slot.durationM} minutes at ${slot.start} should be available`);
    }
  });
  ...
  it('should return false for a slot overlapping with an event', () => {
    ...
    for (const slot of unavailableSlots) {
      ...
      expect(actual).toBe(expected, `Slot with ${slot.durationM} minutes at ${slot.start} should be unavailable`);
    }
  });
});
